...
if 'member_analysis_results' not in st.session_state:
    st.session_state.member_analysis_results = {}
...
# Function to create FirstOrderMemberResponse instance with data from session state
def get_member_response():
    # Extract required data in the format expected by FirstOrderMemberResponse
    ...
    # Create and return the FirstOrderMemberResponse instance
    return FirstOrderMemberResponse(Points=points, Members=members, Loads=loads)

# Main layout
st.title("First Order Analysis")
...
with tab1:
    st.header("Member Displacement Analysis")
    
    member_ids = list(st.session_state.members.keys())
    selected_member_id = st.selectbox("Select Member", member_ids, key="displacement_member")
    selected_member = st.session_state.members[selected_member_id]
    
    if st.button("Calculate Displacement", key="btn_displacement"):
        try:
            # Create member response object
            member_response = get_member_response()
            
            # Calculate displacement
            displacement = member_response.MemberDisplacement(selected_member.Beam_Number)
            
            # Store in session state
            st.session_state.member_analysis_results["displacement"] = displacement
            
            # Display results
            st.success(f"Displacement calculated for Member {selected_member_id}")
            ...
            st.error(f"Error calculating displacement: {str(e)}")

# Tab 2: Member Forces
with tab2:
    st.header("Member Forces Analysis")
    
    force_tab1, force_tab2 = st.tabs(["Local Forces", "Global Forces"])
    
    with force_tab1:
        member_ids = list(st.session_state.members.keys())
        selected_member_id = st.selectbox("Select Member", member_ids, key="local_force_member")
        selected_member = st.session_state.members[selected_member_id]
        
        if st.button("Calculate Local Forces", key="btn_local_force"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                # Store in session state
                st.session_state.member_analysis_results["local_forces"] = forces
                
                # Display results
                st.success(f"Local forces calculated for Member {selected_member_id}")
                ...
                st.error(f"Error calculating local forces: {str(e)}")
    
    with force_tab2:
        member_ids = list(st.session_state.members.keys())
        selected_member_id = st.selectbox("Select Member", member_ids, key="global_force_member")
        selected_member = st.session_state.members[selected_member_id]
        
        if st.button("Calculate Global Forces", key="btn_global_force"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                # Store in session state
                st.session_state.member_analysis_results["global_forces"] = forces
                
                # Display results
                st.success(f"Global forces calculated for Member {selected_member_id}")
                ...
                st.error(f"Error calculating global forces: {str(e)}")
...
with tab3:
    ...
    with bmd_tab1:
        member_ids = list(st.session_state.members.keys())
        selected_member_id = st.selectbox("Select Member", member_ids, key="bmd_member")
        selected_member = st.session_state.members[selected_member_id]
        
        if st.button("Plot Bending Moment Diagram", key="btn_bmd"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                # Store in session state
                st.session_state.member_analysis_results["bmd"] = {
                    "moments": moments,
                    "positions": positions
                }
                
                # Display numerical data in a table if desired
                if st.checkbox("Show Numerical Data", key="show_bmd_data"):
                    data = pd.DataFrame({
                ...
                st.error(f"Error plotting bending moment diagram: {str(e)}")
    
    with bmd_tab2:
        # Dynamic scale factor control with slider and fine adjustment
        col1, col2 = st.columns([3, 1])
        with col1:
            scale_factor = st.slider("Scale Factor", min_value=0.01, max_value=10.0, value=0.5, step=0.01, key="bmd_scale")
        with col2:
        ...
        show_structure = st.checkbox("Show Structure", value=True, key="bmd_show_structure")
        
        if st.button("Plot Global Bending Moment Diagram", key="btn_global_bmd"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                st.error(f"Error plotting global bending moment diagram: {str(e)}")

# Tab 4: Shear Force Diagram
with tab4:
    st.header("Shear Force Diagram")
    
    sfd_tab1, sfd_tab2 = st.tabs(["Individual Member SFD", "Global Structure SFD"])
    
    with sfd_tab1:
        member_ids = list(st.session_state.members.keys())
        selected_member_id = st.selectbox("Select Member", member_ids, key="sfd_member")
        selected_member = st.session_state.members[selected_member_id]
        
        if st.button("Plot Shear Force Diagram", key="btn_sfd"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                # Store in session state
                st.session_state.member_analysis_results["sfd"] = {
                    "shear_forces": shear_forces,
                    "positions": positions
                }
                
                # Display numerical data in a table if desired
                if st.checkbox("Show Numerical Data", key="show_sfd_data"):
                    data = pd.DataFrame({
                ...
                st.error(f"Error plotting shear force diagram: {str(e)}")
    
    with sfd_tab2:
        # Dynamic scale factor control with slider and fine adjustment
        col1, col2 = st.columns([3, 1])
        with col1:
            scale_factor = st.slider("Scale Factor", min_value=0.01, max_value=10.0, value=0.5, step=0.01, key="sfd_scale")
        with col2:
            fine_adjust = st.number_input("Fine Adjust", min_value=0.001, max_value=50.0, value=scale_factor, step=0.001, key="sfd_fine_adjust")
            if fine_adjust != scale_factor:
                scale_factor = fine_adjust
        
        show_structure = st.checkbox("Show Structure", value=True, key="sfd_show_structure")
        
        if st.button("Plot Global Shear Force Diagram", key="btn_global_sfd"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                st.error(f"Error plotting global shear force diagram: {str(e)}")

# Tab 5: Deflection
with tab5:
    st.header("Deflection Analysis")
    
    deflection_tab1, deflection_tab2 = st.tabs(["Individual Member Deflection", "Global Structure Deflection"])
    
    with deflection_tab1:
        member_ids = list(st.session_state.members.keys())
        selected_member_id = st.selectbox("Select Member", member_ids, key="deflection_member")
        selected_member = st.session_state.members[selected_member_id]
        
        if st.button("Plot Member Deflection", key="btn_deflection"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                try:
                    ...
                    # Store in session state
                    st.session_state.member_analysis_results["deflection"] = {
                        "deflections": deflections,
                        "positions": positions
                    }
                    
                    # Display numerical data in a table if desired
                    if st.checkbox("Show Numerical Data", key="show_deflection_data"):
                        data = pd.DataFrame({
                            "Position (m)": positions,
                ...
                st.error(f"Error plotting member deflection: {str(e)}")
    
    with deflection_tab2:
        # Dynamic scale factor control with slider and fine adjustment
        col1, col2 = st.columns([3, 1])
        with col1:
            scale_factor = st.slider("Scale Factor", min_value=0.1, max_value=100.0, value=10.0, step=0.1, key="deflection_scale")
        with col2:
            fine_adjust = st.number_input("Fine Adjust", min_value=0.1, max_value=500.0, value=scale_factor, step=0.1, key="deflection_fine_adjust")
            if fine_adjust != scale_factor:
                scale_factor = fine_adjust
        
        show_structure = st.checkbox("Show Structure", value=True, key="deflection_show_structure")
        
        if st.button("Plot Global Deflection", key="btn_global_deflection"):
            try:
                # Create member response object
                member_response = get_member_response()
                ...
                st.error(f"Error plotting global deflection: {str(e)}")
